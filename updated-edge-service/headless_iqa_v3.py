...
parser.add_argument("--run-id", type=str, default=None, help="Optional run id to use for the output subdirectory.")
args = parser.parse_args()

log_root = args.log_dir or Path(tempfile.gettempdir()) / "thirdeye_iqa_logs"
log_root.mkdir(parents=True, exist_ok=True)
run_id = args.run_id or f"run_{datetime.datetime.now():%Y%m%d_%H%M%S}_{uuid.uuid4().hex[:6]}"
