...
class AestheticScorer:
    ...
    def load(self, device: str) -> None:
        ...
        logger.info(f"Loading AestheticScorer processor from {b_name}")
        self.processor = AutoImageProcessor.from_pretrained(b_name)
        logger.info("✓ Processor loaded")
        
        logger.info(f"Loading AestheticScorer backbone from {b_name}")
        backbone = AutoModel.from_pretrained(b_name)
        logger.info("✓ Backbone loaded")
        
        logger.info("Creating MLPHead")
        head = MLPHead(embed_dim=384)
        logger.info("✓ MLPHead created")

        logger.info("Downloading head checkpoint")
        ...
        logger.info("Loading head state dict")
        head.load_state_dict(torch.load(ckpt_path, map_location=device))
        logger.info("✓ Head state dict loaded")

        logger.info(f"Moving full model to device {device}")
        full_model = DINOAestheticScorer(backbone, head).to(device)
        logger.info("✓ Full model moved to device")
        ...
        logger.info("✓ AestheticScorer fully loaded")
